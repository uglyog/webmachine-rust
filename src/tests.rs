...
#[derive(Clone, Debug)]
struct TestWebmachineResource {

}

impl WebmachineResource for TestWebmachineResource {
...
fn path_matcher_test() {
    let dispatcher = WebmachineDispatcher::new(
        btreemap!{
            s!("/") => TestWebmachineResource { },
            s!("/path1") => TestWebmachineResource { },
            s!("/path2") => TestWebmachineResource { },
            s!("/path1/path3") => TestWebmachineResource { }
        }
    );
...
#[test]
fn dispatcher_returns_None_if_there_is_no_matching_resource() {
    let mut context = WebmachineContext::default();
    let dispatcher = WebmachineDispatcher::new(
        btreemap!{ s!("/some/path") => TestWebmachineResource { } }
    );
    expect!(dispatcher.matching_resource(&mut context)).to(be_none());
}
